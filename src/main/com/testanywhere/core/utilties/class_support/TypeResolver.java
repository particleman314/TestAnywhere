...
public final class TypeResolver
{
     ...
     * @return argument for {@code type} else if no type arguments are declared
     ...
     * @param subType to extract type variable information from
     * @return argument for {@code genericType} else if no type arguments are
     *         declared
     * @throws IllegalArgumentException if more or less than one argument is resolved for the
     *           {@code genericType}
     */
        ...
        if (GET_CONSTANT_POOL != null)
        {
            try
            {
                ...
                for (Method m : functionalInterface.getMethods()) {
                    if (!Modifier.toString(m.getModifiers()&Modifier.methodModifiers()).equals("default") &&
                            !Modifier.isStatic(m.getModifiers()) && !m.isBridge())
                    {
