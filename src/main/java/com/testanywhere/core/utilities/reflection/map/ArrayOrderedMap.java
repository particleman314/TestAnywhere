...
public class ArrayOrderedMap<K, V> extends HashMap<K, V> implements OrderedMap<K, V>
{
    ...
    public Entry<K, V> getEntry(int position) {
        if (position <= -1 || position >= this.list.size())
            throw new IllegalArgumentException("Value is out of bounds for the map size: " + list.size());
        K key = this.list.get(position);
        Entry<K, V> entry = new SimpleEntry<K,V>(key, this.get(key));
        return entry;
    }
    ...
    public Set<Map.Entry<K,V>> entrySet()
    {
        ...
        return (es != null) ? es : (this.entrySet = (Set<Map.Entry<K,V>>) (Set) new EntrySet());
    }
