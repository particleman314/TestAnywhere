...
import com.testanywhere.core.utilities.reflection.beanutils.FieldAdapter;
...
public class DeepUtils {
    ...
    static
    {
        ...
        DeepUtils.logger = Logger.getLogger(DeepUtils.class);
        DeepUtils.getInstance();
    }
    ...
    {
        if ( ! DeepUtils.isInitialized ) {
            DeepUtils.isInitialized = true;
            DeepUtils.initialize();
        }
        return DeepUtilsHolder.INSTANCE;
    }

    private static class DeepUtilsHolder
    {
        public static final DeepUtils INSTANCE = new DeepUtils();
    }
    ...
    public <T> T deepClone(T object, int maxDepth, String[] fieldNamesToSkip)
    {
        ...
        T clone = (T) this.internalDeepClone(object, CopyDestination.ORIGINAL, maxDepth, skip, 0, true, false);
        return clone;
    }
     ...
     */
    public boolean deepCopy(Object orig, Object dest, int maxDepth, String[] fieldNamesToSkip, boolean ignoreNulls)
    {
        Set<String> skip = ArrayUtils.makeSetFromArray(fieldNamesToSkip);
        return this.internalDeepCopy(orig, dest, maxDepth, skip, ignoreNulls, true);
    }
    ...
    {
        Set<String> skip = ArrayUtils.makeSetFromArray(fieldNamesToSkip);
        Object clone = this.internalDeepClone(object, CopyDestination.MAP, maxDepth, skip, 0, ignoreNulls, ignoreTransient);
        ...
        if (clone != null)
        {
            ...
            else
            {
                ...
                if (initialKey == null) initialKey = "data";
    ...
    {
        if (object == null || properties == null) throw new IllegalArgumentException("object and properties cannot be null");
        Map<String, Class<?>> fieldTypes = this.getFieldUtils().getFieldTypes(object.getClass(), FieldsFilter.ALL);
                ...
                if (targetType != null)
                {
                    ...
                    try
                    {
                        if ( ConstructorUtils.isClassSimple(targetType) )
                        {
                            ...
                            this.getFieldUtils().setFieldValue(object, fieldName, value, true);
                        }
                        else
                        {
                            // value is a bean so we will dig into it further
                            Object dest = this.getFieldUtils().getFieldValue(object, fieldName);
                            if (dest == null)
                            {
                                ...
                                {
                                    dest = this.getConstructorUtils().constructClass(targetType);
                                }
                                this.getFieldUtils().setFieldValue(object, fieldName, dest, true);
                            }
                            this.internalDeepCopy(value, dest, 5, null, false, true);
                        }
    ...
    {
        if (object == null || params == null) throw new IllegalArgumentException("object and params cannot be null");
        List<String> writtenFields = new ArrayList<String>();
        ...
        for (Entry<String, String[]> entry : params.entrySet())
        {
            ...
            if (fieldName != null && ! "".equals(fieldName))
            {
                ...
                try
                {
                    ...
                    {
                        Object toSet = value;
                        if (value.length == 0) toSet = "";
                        else if (value.length == 1) toSet = value[0];

                        this.getFieldUtils().setFieldValue(object, fieldName, toSet, true);
                        writtenFields.add(fieldName); // record that the value was written successfully
                    }
                }
                catch (RuntimeException e)
                    ...
                    try {
                        ...
                        if (beanClass.isArray())
                        {
                            ...
                            for (int i = 0; i < length; i++)
                            {
                                ...
                                Object clone = this.internalDeepClone(value, dest, maxDepth, null, currentDepth, ignoreNulls, ignoreTransient);
                        ...
                        else if (Collection.class.isAssignableFrom(beanClass))
                        {
                            ...
                            copy = this.getConstructorUtils().constructClass(beanClass); // make new list
                            for (Object element : (Collection) bean)
                            {
                                Object clone = this.internalDeepClone(element, dest, maxDepth, null, currentDepth, ignoreNulls, ignoreTransient);
                        ...
                        else if (Map.class.isAssignableFrom(beanClass))
                        {
                            ...
                            copy = this.getConstructorUtils().constructClass(beanClass); // make new map
                            for (Object key : ((Map) bean).keySet())
                            {
                                if ( fieldNamesToSkip != null
                                        && fieldNamesToSkip.contains(key) ) continue; // skip to next
                                Object value = ((Map<?,?>) bean).get(key);
                                if (value == null && ignoreNulls) continue;
                                try
                                {
                                    ((Map) copy).put(key,
                                            this.internalDeepClone(value, dest, maxDepth, null, currentDepth, ignoreNulls, ignoreTransient) );
                                }
                        ...
                        else if (getFieldAdapter().isAdaptableClass(beanClass))
                        {
                            ...
                            if (CopyDestination.MAP.equals(dest)) copy = new ArrayOrderedMap<String, Object>();
                            else copy = this.getFieldAdapter().newInstance(bean); // make new dynabean

                            List<String> propertyNames = this.getFieldAdapter().getPropertyNames(bean);
                            for (String name : propertyNames)
                            {
                                if ( fieldNamesToSkip != null
                                        && fieldNamesToSkip.contains(name) ) continue; // skip to next
                                try
                                {
                                    Object value = this.getFieldAdapter().getSimpleValue(bean, name);
                                    if (value == null && ignoreNulls) continue;
                                    ...
                                    {
                                        this.getFieldUtils().setFieldValue(copy, name,
                                                this.internalDeepClone(value, dest, maxDepth, null, currentDepth, ignoreNulls, ignoreTransient) );
                                    }
                                }
                        ...
                        else
                        {
                            ...
                            if (CopyDestination.MAP.equals(dest))
                            {
                                ...
                                if (ignoreTransient) filter = FieldsFilter.SERIALIZABLE;
                                else filter = FieldsFilter.READABLE;
                            }
                            else
                            {
                                copy = this.getConstructorUtils().constructClass(beanClass); // make new bean
                            }
                            ClassFields<?> cf = this.getFieldUtils().analyzeClass(beanClass);
                            Map<String, Class<?>> types = cf.getFieldTypes(filter);
                            for (String name : types.keySet())
                            {
                                if (ClassFields.FIELD_CLASS.equals(name)) continue; // No point in trying to set/get an object's class
                                if ( fieldNamesToSkip != null
                                        && fieldNamesToSkip.contains(name) ) continue; // skip to next
                                try
                                {
                                    Object value = this.getFieldUtils().getFieldValue(bean, name);
                                    if (value == null && ignoreNulls) continue;
                                    if (CopyDestination.MAP.equals(dest))
                                    {
                                        ((Map<String, Object>) copy).put(name, 
                                ...
                                } catch (FieldnameNotFoundException | FieldGetValueException |
                                         FieldSetValueException | IllegalArgumentException e) {
     ...
     */
    @SuppressWarnings("unchecked")
    protected boolean internalDeepCopy(Object orig, Object dest, int maxDepth, Set<String> fieldNamesToSkip, boolean ignoreNulls, boolean autoConvert)
    {
        if (orig == null || dest == null) throw new IllegalArgumentException("original object and destination object must not be null, if you want to clone the object then use the clone method");
        ...
        if (ConstructorUtils.getImmutableTypes().contains(destClass)) throw new IllegalArgumentException("Cannot copy to an immutable object ("+destClass+")");
        else if (ConstructorUtils.isClassSpecial(origClass)) throw new IllegalArgumentException("Cannot copy a special object ("+origClass+")");
        ...
        if (autoConvert && ! ConstructorUtils.isClassBean(origClass) ) {
            ...
            try
            {
                orig = this.getConversionUtils().convert(orig, destClass);
                origClass = destClass;
            }
        ...
        try
        {
            if (origClass.isArray())
            {
                ...
                {
                    //if ( Array.getLength(dest) < 1 ) throw new IllegalArgumentException("destination is empty");
                    try
                    {
                        for (int i = 0; i < Array.getLength(orig); i++)
                        {
                            ...
                            if (! ignoreNulls || value != null)
                                 Array.set(dest, i,
                                        this.internalDeepClone(value, CopyDestination.ORIGINAL, maxDepth, null, currentDepth, ignoreNulls, false));
                        }
                    }
            ...
            {
                List<String> propertyNames = this.getFieldAdapter().getPropertyNames(orig);
                for (String name : propertyNames)
                {
                    if ( fieldNamesToSkip != null
                            && fieldNamesToSkip.contains(name) ) continue; // skip to next
                    try
                    {
                        Object value = this.getFieldAdapter().getSimpleValue(orig, name);
                        if (!ignoreNulls || value != null)
                            this.getFieldUtils().setFieldValue(dest, name,
                                    this.internalDeepClone(value, CopyDestination.ORIGINAL, maxDepth, null, currentDepth, ignoreNulls, false));
                    }
                    catch (FieldnameNotFoundException | IllegalArgumentException e)
                    {
            ...
            {
                // regular javabean
                Map<String, Object> values = this.getFieldUtils().getFieldValues(orig, FieldsFilter.READABLE, false);
                for (Entry<String, Object> entry : values.entrySet())
                {
                    String name = entry.getKey();
                    if (ClassFields.FIELD_CLASS.equals(name)) continue; // No point in trying to set an object's class
                    if ( fieldNamesToSkip != null
                            && fieldNamesToSkip.contains(name) ) continue; // skip to next
                    try
                    {
                        Object value = this.getFieldUtils().getFieldValue(orig, name);
                        if (! ignoreNulls || value != null)
                            this.getFieldUtils().setFieldValue(dest, name,
                                    this.internalDeepClone(value, CopyDestination.ORIGINAL, maxDepth, null, currentDepth, ignoreNulls, false));
                    } catch (FieldnameNotFoundException | FieldGetValueException |
    ...
    {
        return "CPUT:" + this.getClassDataCacher();
    }
}
