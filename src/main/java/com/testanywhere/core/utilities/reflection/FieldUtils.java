...
import com.testanywhere.core.utilities.reflection.ClassProperty.IndexedProperty;
import com.testanywhere.core.utilities.reflection.ClassProperty.MappedProperty;
import com.testanywhere.core.utilities.reflection.beanutils.DefaultResolver;
import com.testanywhere.core.utilities.reflection.beanutils.FieldAdapter;
import com.testanywhere.core.utilities.reflection.beanutils.FieldAdapterManager;
...
public class FieldUtils {
    ...
    private FieldAdapterManager fieldAdapterManager;
    private Range<Integer> ffm;
    ...
    static
    {
        ...
        FieldUtils.logger = Logger.getLogger(FieldUtils.class);
    ...
    {
        if ( ! FieldUtils.isInitialized ) {
            FieldUtils.isInitialized = true;
            FieldUtils.initialize();
        }
        return FieldUtilsHolder.INSTANCE;
    }

    private static class FieldUtilsHolder
    {
    ...
    {
        //super();
        this.ffm = new Range<>(ClassFields.FieldFindMode.HYBRID.ordinal(), ClassFields.FieldFindMode.ALL.ordinal());
    ...
    {
        this.nameResolver = ( resolver != null ) ? resolver : new DefaultResolver();
    }
    ...
    public <T> ClassFields<T> analyzeClass(Class<T> cls)
    {
        ...
        ClassFields<T> cf = this.getClassDataCacher().getClassFields(cls);
        return cf;
    }

    public <T> ClassFields<T> analyzeClass(Class<T> cls, ClassFields.FieldFindMode mode)
    {
        if ( cls == null || this.ffm.outOfBounds(mode.ordinal()) ) return null;
        ClassFields<T> cf = this.getClassDataCacher().getClassFields(cls, mode);
        return cf;
    }
    ...
    {
        if (type == null || name == null) throw new IllegalArgumentException("type and name must not be null");

        // get the nested class or return Object.class as a cop out
        while ( this.getResolver().hasNested(name) )
        {
            String next = this.getResolver().next(name);
            Class<?> nestedClass;
            if (Object.class.equals(type)
                    || Map.class.isAssignableFrom(type)
                    || this.getResolver().isMapped(next)
            ...
            else
            {
                ...
                ClassFields<?> cf = this.analyzeClass(type);
        ...
        else
        {
            ...
            catch (FieldnameNotFoundException fnfe)
            {
                ...
                Field field = this.getFieldIfPossible(cd, name);
                if (field == null)
                {
    ...
    {
        if (obj == null || name == null) throw new IllegalArgumentException("obj and name must not be null");
        if (Class.class.equals(obj))
        {
            // this is a class so we should pass it over to the other method
        ...
        String targetName = this.getResolver().getProperty(name); // simple name of target field
        // get the type
        Class<?> fieldType;
        ...
        {
            // normal object
            ClassFields<?> cf = this.analyzeObject(obj);
            try
            {
    ...
    {
        ClassFields<?> cf = this.analyzeClass(type, findFieldFindMode(filter));
    ...
    {
        ClassFields<T> cf = this.analyzeClass(cls);
        return cf.getFieldNames();
    }

    public <T> List<String> getFieldNames(Class<T> cls, FieldsFilter filter)
    {
        ClassFields<T> cf = this.analyzeClass(cls, findFieldFindMode(filter));
        return cf.getFieldNames();
    }
    ...
    public Map<String, Object> getFieldValues(Object obj, FieldsFilter filter, boolean includeClassField)
    {
        ...
        {
            Map<String, Class<?>> types = this.getFieldTypes(obj.getClass(), filter);
            ...
            for (String name : types.keySet())
            {
                try
                {
                    Object o = this.getFieldValue(obj, name);
                    values.put(name, o);
                }
    ...
    {
        if (obj == null) throw new IllegalArgumentException("obj cannot be null");
        if (! TextManager.validString(name)) throw new IllegalArgumentException("field name cannot be null or blank");

        // Resolve nested references
        Holder holder = this.unpackNestedName(name, obj, false);
        name = holder.getName();
    ...
    {
        Object o = this.getFieldValue(obj, name);
        T value = this.getConversionUtils().convert(o, asType);
        return value;
    }
    ...
    public boolean setFieldValue(Object obj, String name, Object value) {
        return this.setFieldValue(obj, name, value, true);
    }
     ...
     */
    @SuppressWarnings("unchecked")
    public boolean setFieldValue(Object obj, String name, Object value, boolean autoConvert)
    {
        if (obj == null) throw new IllegalArgumentException("obj cannot be null");
        if (!TextManager.validString(name)) throw new IllegalArgumentException("field name cannot be null or blank");

        // Resolve nested references
        Holder holder = this.unpackNestedName(name, obj, true);
        name = holder.getName();
        obj = holder.getObject();
        ...
        if (autoConvert)
        {
            ...
            Class<?> type = this.getFieldType(obj, name);
    ...
    {
        if (indexedObject == null) throw new IllegalArgumentException("Invalid indexedObject, cannot be null");
                ...
                if (nestedBean == null)
                {
                    ...
                    if (autoCreate)
                    {
                        ...
                        try
                        {
                            ...
                            this.setFieldValue(obj, next, nestedBean, false); // need to put this new object into the parent
                        }
                        catch (RuntimeException e)
                        {
    ...
    protected Object getSimpleValue(Object obj, String name)
    {
        ...
        else
        {
            ...
            catch (FieldnameNotFoundException fnfe)
            {
                ...
                ClassData<?> cd = cf.getClassData();
                Field field = this.getFieldIfPossible(cd, name);
                if (field == null) throw new FieldnameNotFoundException("Could not find field with name ("+name+") on object (" + obj + ") after extended look into non-visible fields", fnfe);
                try
                {
                    value = field.get(obj);
                }
    ...
    protected Object getIndexedValue(Object obj, String name) {
        ...
        try
        {
            ...
            if (index < 0) throw new IllegalArgumentException("Could not find index in name (" + name + ")");
        ...
        else
        {
            ...
            else
            {
                ...
                ClassFields cf = this.analyzeObject(obj);
                ClassProperty cp = cf.getClassProperty(name);
                if (! cp.isIndexed()) throw new IllegalArgumentException("This field ("+name+") is not an indexed field");
                isArray = cp.isArray();
    ...
    protected Object getMappedValue(Object obj, String name)
    {
        ...
        try
        {
            ...
            if (key == null) throw new IllegalArgumentException("Could not find key in name (" + name + ")");
        ...
        else
        {
            ...
            else
            {
                ...
                ClassFields cf = this.analyzeObject(obj);
                ClassProperty cp = cf.getClassProperty(name);
                if (! cp.isMapped())  throw new IllegalArgumentException("This field ("+name+") is not an mapped field");
                ...
                {
                    Object o = this.findFieldValue(obj, cp);
                    if (! Map.class.isAssignableFrom(o.getClass())) throw new IllegalArgumentException("Field (" + name + ") does not appear to be a map (not instance of Map)");
                    map = (Map) o;
                }
            }
    ...
    protected Object getValueOfMap(Map map, String name)
    {
        ...
        if (resolver.isMapped(name))
        {
            ...
            if ( ! TextManager.validString(propName) ) name = resolver.getKey(name);
        }
    ...
    protected Object findFieldValue(Object obj, ClassProperty cp)
    {
        ...
        if (cp == null) throw new IllegalArgumentException("ClassProperty cannot be null");
        ...
        else
        {
            ...
            if ( getter == null ) throw new IllegalArgumentException("Getter method failure getting value for name ("+cp.getFieldName()+") from: " + obj);
                    ...
                    if (indexedObject == null)
                    {
                        ...
                        try
                        {
                            ...
                            { // List
                                ...
                                if (type.isInterface()) indexedObject = new ArrayList(index+1);
                                else indexedObject = type.newInstance();
                            }
                ...
                if (isArray)
                {
                    ...
                    try
                    {
                        ...
                        if (index >= length)
                        {
                            ...
                            this.setSimpleValue(obj, name, indexedObject); // need to put the array back into the object
                        }
    ...
    protected void setMappedValue(Object obj, String name, Object value)
    {
        ...
        else
        {
            ...
            {
                // set value in map
                if (map == null) throw new IllegalArgumentException("Mapped object is null, cannot set value for key ("+key+") on field ("+name+")");
                // set value on map
                try
                {
                    map.put(key, value);
    ...
    public String toString()
    {
        ...
        sb.append("FIELD" + TextManager.STR_OUTPUTSEPARATOR);
        sb.append("RES=" + this.getResolver() + TextManager.STR_OUTPUTSEPARATOR);
        return sb.toString();
    }
}
