...
import com.testanywhere.core.utilities.reflection.beanutils.Resolver;
...
public class ReflectUtils
{
    ...
    static
    {
        ...
        ReflectUtils.logger = Logger.getLogger(ReflectUtils.class);
        ReflectUtils.getInstance();
        DeepUtils.getInstance();
        ClassDataCacher.getInstance();
        ConstructorUtils.getInstance();
        FieldUtils.getInstance();
    }

    public static ReflectUtils getInstance()
    {
        if ( ! ReflectUtils.isInitialized ) {
            ReflectUtils.isInitialized = true;
            ReflectUtils.initialize();
        }
    ...
    {
        super();
        ReflectUtils.setFieldFindMode(FieldFindMode.HYBRID);
    }
    // setter passthroughs
     ...
     * @see FieldFindMode
     */
    public static void setFieldFindMode(FieldFindMode fieldFindMode)
    {
        ReflectUtils.getClassDataCacher().setFieldFindMode(fieldFindMode);
    }
    ...
    public static void setReflectionCache(Map<Class<?>, ClassFields> reflectionCache)
    {
        ReflectUtils.getClassDataCacher().setReflectionCache(reflectionCache);
    }
    ...
    {
        ReflectUtils.getFieldUtils().setResolver(resolver);
    }
     ...
     */
    public static void setConverters(Map<Class<?>, Converter<?>> converters)
    {
        ReflectUtils.getConversionUtils().setConverters(converters);
    }
    ...
    {
        if ( type == ReflectConstants.NO_CLASS ) return false;
        if ( converter == null ) converter = new StringConverter();
        return this.getConversionUtils().addConverter(type, converter);
    }
    ...
    {
        this.getClassDataCacher().setIncludeClassField(includeClassField);
    }
    ...
    {
        if ( beanClass == ReflectConstants.NO_CLASS ) return null;
        return this.getFieldUtils().analyzeClass(beanClass);
    }
    ...
    {
        if ( object == ReflectConstants.NO_OBJECT || ! TextManager.validString(fieldName) ) return ReflectConstants.NO_OBJECT;
        return this.getFieldUtils().getFieldValue(object, fieldName);
    }
    ...
    {
        if ( object == ReflectConstants.NO_OBJECT || ! TextManager.validString(fieldName) ) return null;
        if ( annotationClass == null ) return this.getFieldValue(object, fieldName);
        ...
        if (annotationClass != null)
        {
            ...
            String annotatedField = this.getFieldNameWithAnnotation(elementClass, annotationClass);
    ...
    public String getFieldValueAsString(Object object, String fieldName, Class<? extends Annotation> annotationClass)
    {
        ...
        Object value = this.getFieldValue(object, fieldName, annotationClass);

        if (value != null) return this.getConversionUtils().convert(value, String.class); //value.toString();
        return null;
    }
    ...
    {
        if ( object == ReflectConstants.NO_OBJECT || ! TextManager.validString(fieldName) ) return false;
        return this.getFieldUtils().setFieldValue(object, fieldName, value);
    }
     ...
     * @throws UnsupportedOperationException if the value could not be converted to the field type
     */
    public boolean setFieldValue(Object object, String fieldName, Object value, boolean autoConvert)
    {
        if ( object == ReflectConstants.NO_OBJECT || ! TextManager.validString(fieldName) ) return false;
        return this.getFieldUtils().setFieldValue(object, fieldName, value, autoConvert);
    }
    ...
    {
        if ( type == ReflectConstants.NO_CLASS ) return new TreeMap<>();
    ...
    {
        if ( type == null ) return new TreeMap<>();
        Map<String, Class<?>> types = this.getFieldUtils().getFieldTypes(type, filter);
        return types;
    }
    ...
    {
        if ( type == null || ! TextManager.validString(fieldName) ) return null;
        return this.getFieldUtils().getFieldType(type, fieldName);
    }
    ...
    public Map<String, Object> getObjectValues(Object object)
    {
        ...
        return this.getObjectValues(object, FieldsFilter.READABLE, false);
    }
    ...
    {
        if ( object == null ) return new TreeMap<>();
        return this.getFieldUtils().getFieldValues(object, filter, includeClassField);
    }
    ...
    public String getFieldNameWithAnnotation(Class<?> elementClass, Class<? extends Annotation> annotationClass)
    {
        ...
        if (annotationClass == null) throw new IllegalArgumentException("the annotationClass must not be null");

        ClassFields<?> cf = getFieldUtils().analyzeClass(elementClass);
        return cf.getFieldNameByAnnotation(annotationClass);
    }
     ...
     */
    public boolean copy(Object orig, Object dest, int maxDepth, String[] fieldNamesToSkip, boolean ignoreNulls)
    {
        if ( orig == null || dest == null ) return false;
        if ( maxDepth < 0 ) maxDepth = 0;
        return this.getDeepUtils().deepCopy(orig, dest, maxDepth, fieldNamesToSkip, ignoreNulls);
    }
     ...
     */
    public Map<String, Object> map(Object object, int maxDepth, String[] fieldNamesToSkip,
                                   boolean ignoreNulls, boolean ignoreTransient, String initialKey)
    {
        if ( object == null ) return null;
        if ( maxDepth < 0 ) maxDepth = 0;
        return this.getDeepUtils().deepMap(object, maxDepth, fieldNamesToSkip, ignoreNulls, ignoreTransient, initialKey);
    }
     ...
     */
    public Collection<String> populate(Object object, Map<String, Object> properties)
    {
        if ( object == null ) return null;
     ...
     * @throws IllegalArgumentException if the arguments are invalid
     */
    public Collection<String> populateFromParams(Object object, Map<String, String[]> params)
    {
        if ( object == null ) return null;
        if ( params == null || params.isEmpty() ) return new ArrayList<>();
    ...
    public <T> T convert(Object object, Class<T> type)
    {
        ...
        T convert = this.getConversionUtils().convert(object, type);
        return convert;
    }
    ...
    {
        if ( ! TextManager.validString(methodName) ) return null;
    ...
    {
        String md5 = "";
        if ( TextManager.validString(text) ) {
            MessageDigest md;
            try {
        ...
        if (maxLength > 0 && md5.length() > maxLength) md5 = md5.substring(0, maxLength);
        return md5;
    }
    ...
    {
        if (map == null || map.isEmpty() || keys == null || keys.length < 1 ) return null;
    ...
    public static boolean pdNameCompare(String name, String fieldName) {
        if ( ! TextManager.validString(name) || ! TextManager.validString(fieldName) ) return false;

        boolean match = false;
        if (name.length() == 0 || fieldName.length() == 0) {
            match = false;
    ...
    {
        StringBuilder sb = new StringBuilder();
        sb.append("REFLECT" + TextManager.STR_OUTPUTSEPARATOR);
        sb.append("CDC=" + this.getClassDataCacher() + TextManager.STR_OUTPUTSEPARATOR);
        sb.append("FU=" + this.getFieldUtils() + TextManager.STR_OUTPUTSEPARATOR);
        sb.append("CU=" + this.getConversionUtils() + TextManager.STR_OUTPUTSEPARATOR);
    ...
    private static FieldUtils getFieldUtils() {
        return FieldUtils.getInstance();
    }

    private static ConstructorUtils getConstructorUtils() {
        return ConstructorUtils.getInstance();
    }

    private static ClassDataCacher getClassDataCacher() {
        return ClassDataCacher.getInstance();
    }

    private static ConversionUtils getConversionUtils() {
        return ConversionUtils.getInstance();
    }

    private static DeepUtils getDeepUtils() {
