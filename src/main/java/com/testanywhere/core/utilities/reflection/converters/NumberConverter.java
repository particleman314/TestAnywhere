...
public class NumberConverter implements Converter<Number>
{
    ...
    {
        if ( targetType == null ) throw new IllegalArgumentException("Type cannot be null");
        ...
        if (value instanceof Number) {
            return (T) NumberConverter.toNumber(sourceType, targetType, (Number)value);
        }

        // Handle Boolean
        if (value instanceof Boolean) {
            return (T) NumberConverter.toNumber(sourceType, targetType, ((Boolean)value).booleanValue() ? 1 : 0);
        }

        // Handle Date
        if (value instanceof Date) {
            long time = ((Date)value).getTime();
            return (T) NumberConverter.toNumber(Long.class, targetType, time);
        }
        ...
        Number number = NumberConverter.toNumber(sourceType, targetType, stringValue);
        return (T) number;
    }
    ...
    private static Number toNumber(Class<?> sourceType, Class<?> targetType, Number value)
    {
        ...
        if ( ConstructorUtils.classEquals(targetType, value.getClass()) ) return value;
