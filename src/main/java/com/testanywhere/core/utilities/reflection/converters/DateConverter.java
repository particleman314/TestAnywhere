...
public class DateConverter extends BaseDateFormatHolder implements Converter<Date>
{
    ...
    public Date convert(Object value) {
        return DateConverter.convertToType(Date.class, value, this.getDateFormats());
    }
    ...
    {
        return DateConverter.convertToType(targetType, value, null);
    }
    ...
    public static <T> T convertToType(Class<T> targetType, Object value, DateFormat[] dateFormats) {
        if (value == null) throw new IllegalArgumentException("value to convert cannot be null");
        ...
        if (stringValue.length() == 0)
        {
            ...
            if (defaultValue == null) throw new UnsupportedOperationException("Date convert failure: Could not convert empty string to target ("+targetType+") or get a default value for it");
        ...
        // Parse the Date/Time
        Calendar calendar = DateConverter.parse(stringValue, dateFormats);
        if (calendar != null)
        {
            long time = ((Calendar)value).getTimeInMillis();
            return (T) toDate(targetType, time);
    ...
    {
        // java.util.Date
        if (type.equals(Date.class)) return new Date(value);

        // java.sql.Date
        if (type.equals(java.sql.Date.class)) return new java.sql.Date(value);

        // java.sql.Time
        if (type.equals(java.sql.Time.class)) return new java.sql.Time(value);

        // java.sql.Timestamp
        if (type.equals(java.sql.Timestamp.class)) return new java.sql.Timestamp(value);
     ...
     */
    private static Calendar parse(String value, DateFormat[] dateFormats)
    {
        ...
        if (dateFormats != null && dateFormats.length > 0)
        {
            ...
            {
                calendar = DateConverter.parse(value, format);
        ...
        if (calendar == null)
        {
            ...
            try
            {
                ...
                if (date != null) calendar = format.getCalendar();
            }
     ...
     */
    private static Calendar parse(String value, DateFormat format)
    {
